...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const boundedCosine = Math.max(-1, Math.min(1, cosineTheta));
  const angle = Math.acos(boundedCosine);
  return angle;
}
...
function roundToPowerOfTen(num, pow) {
  const roundToPower = Math.round(num / 10 ** pow) * 10 ** pow;
  return roundToPower;
}
...
function getIntegerOnString(str, base) {
  const parseSum = Number.parseInt(str, base);
  return parseSum.isNumber ? NaN : parseSum;
}
